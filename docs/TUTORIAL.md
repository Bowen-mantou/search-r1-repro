...
> | 符号 | 含义 |
> |---|---|
> | πᶿ / ∇ᶿ | 当前策略 / 对参数 θ 的梯度。⚠️ 标准写法是 π_θ、∇_θ（θ 在**下标**），
> 因 Unicode 没有"下标 θ"字符，文中用上标修饰字母 ᶿ 顶替——请读作下标 θ |
...
### 1.5.1 动机：长度混进了 advantage 的计算

GRPO 的 advantage 只比较 reward，而 reward 的**测量噪声随长度放大**
（§1.5.2 的 √L 论证）。后果有两层：

1. **噪声层面**：长轨迹的 reward 是"质量 + 更多噪声"的混合，优势排序
   会被长度噪声污染——同样质量的轨迹，长的那个优势更像"掷骰子"；
2. **行为层面**：长链探索（多搜几轮）失败率高，若失败的长轨迹被
   **满额惩罚**，策略会学到"永远别试长链"——过早放弃，多跳推理学不会。

LATA 的解法是**双向减震**：把长轨迹的优势在两个方向同时压缩
（除以 √L），好也好得没那么突出、坏也坏得没那么惨——它**不奖励长**，
它只是"对低信噪比的测量打折"，把长度从优势排序里剥离。
保护的是"长链探索不被严厉惩罚打断"；代价是"正确长轨迹的强化也
变弱"（这是取舍，不是免费的）。
